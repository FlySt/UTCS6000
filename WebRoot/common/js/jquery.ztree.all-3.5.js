...
(function($){
	...
	_initNode = function(setting, level, n, parentNode, isFirstNode, isLastNode, openFlag) {
		if (!n) return;
		var cls = "";
		if(n.type != null && n.type != undefined && n.type != ""){
			cls = n.type.toLocaleLowerCase();
		}else{
			if(n.id){
				var tempID = n.id.toString();
				cls = tempID.substring(0,tempID.lastIndexOf('_')).toLocaleLowerCase();
			}
		}
		cls = $.trim(cls);
		if(cls != null && cls != ""){
...
//获取节点的ID和子节点的所有ID
function getNodeIdAndSonIds(treeObj,id){
	nodeIds.push(id);
	var node = treeObj.getNodeByParam("id",id,null) ;
	if(node){
		getAllChildrenNodes(node,nodeIds);
	}
}


	//判断对象是否存在某一数组
	function isObjExist(obs,ob){
		var b = false;
		if(obs){
		...
		if(treeNode.isParent){
			 ...
			 if (childrenNodes) {
		            for (var i = 0; i < childrenNodes.length; i++) {
		            	...
		            	result.push(childrenNode.id);
		            	getAllChildrenNodes(childrenNode,result);
		            }
			 }
		}
	}